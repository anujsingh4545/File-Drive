    ...
    <div className=" w-full overflow-y-scroll h-[calc(100lvh-198px)]  text-white ">
      {LoadingFile ? (
        ...
        File.filter((file: any) => {
          return file.favourite == true && file.filetype.includes(searchText) && (sortFile === "all" || (sortFile === "photo" && (file.filetype === "png" || file.filetype === "jpg" || file.filetype === "jpeg" || file.filetype === "image")) || (sortFile === "csv" && file.filetype === "xlxx") || (sortFile === "pdf" && file.filetype === "pdf") || (sortFile === "docx" && file.filetype === "docx"));
        }).length === 0 ? (
          <Empty />
        ) : (
          <main className={` grid   ${fileorder ? "grid-cols-1  md:grid-cols-2 lg:grid-cols-3" : "grid-cols-1"}  gap-x-3 gap-y-3 px-3 md:px-5 py-3 md:py-5 `}>
            {File.map((file: any) => {
              if (file.favourite == true && file.filetype.includes(searchText) && (sortFile === "all" || (sortFile === "photo" && (file.filetype === "png" || file.filetype === "jpg" || file.filetype === "jpeg" || file.filetype === "image")) || (sortFile === "csv" && file.filetype === "xlxx") || (sortFile === "pdf" && file.filetype === "pdf") || (sortFile === "docx" && file.filetype === "docx"))) {
